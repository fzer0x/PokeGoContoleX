...
# [NEW] Latest Release: https://github.com/fzer0x/LocationJoystick_NEW/releases

![Screenshot](https://github.com/fzer0x/LocationJoystick/blob/main/screenshot2.jpg?raw=true)
...
Location Joystick 是一个用于位置模拟的 Xposed 模块。它提供了一个摇杆悬浮窗以及多种与位置相关的设置，以增强模拟体验。

⚠️ **[新功能] 无需 Root/Xposed 功能（模拟定位）** ⚠️
...
# 最新版本: https://github.com/fzer0x/LocationJoystick-Premium/releases

![截图](https://github.com/fzer0x/PokeGoContoleX/blob/main/screenshot2.jpg?raw=true)

## 功能

- **摇杆悬浮窗：** 使用可自定义的摇杆进行实时移动。
- **位置模拟：** 精确模拟你的 GPS 位置。
- **GNSS 模拟：** 完整的 GNSS 状态模拟（卫星数量、SVID、C/N0 等），用于绕过高级检测。
- **随机化：** 为你的位置添加随机半径，使移动更加自然。
- **收藏夹：** 保存和管理你喜爱的位置，实现快速传送。
- **可自定义界面：**
    - 调整摇杆和小地图透明度（alpha）。
    - 可配置摇杆速度。
    - 摇杆位置持久化保存。
...
该工具仅用于教育和开发目的。使用风险自负。

## 许可证

本项目基于 GPL-3.0 许可证授权 - 详情请参阅 [LICENSE](LICENSE) 文件。
